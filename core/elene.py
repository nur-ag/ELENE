...
class ELENEEmbedder(nn.Module):
    def __init__(self,
            max_degree=50,
            max_distance=3,
            use_elene_degrees=True,
        ...
        self._num_degree_types = 3 if use_elene_degrees else 1
        if not self.joint_embeddings:
            if given_embedders is not None:
                ...
                ) if self.use_elene_degrees else None
    ...
    def compute_node_embedding(self, data):
        ...
        if self.joint_embeddings:
            ...
            node_elene_emb = torch.cat([node_degrees_emb, node_dist_emb], dim=-1)
        return node_elene_emb
    ...
    def compute_edge_embedding(self, data):
        ...
        if self.joint_embeddings:
            ...
            edge_elene_emb = edge_emb_degree_a + edge_emb_degree_b
        else:
            edge_emb_degree_a = torch.cat([
            ...
            edge_elene_emb = torch.cat([edge_emb_degree_a + edge_emb_degree_b, edge_delta_emb_a + edge_delta_emb_b], dim=-1)
        return edge_elene_emb

    def forward(self, data):
        node_elene_emb = self.compute_node_embedding(data)
        if self.node_only:
            return node_elene_emb
        edge_elene_emb = self.compute_edge_embedding(data)
        return node_elene_emb, edge_elene_emb


class ELENEMessagePasser(nn.Module):
    def __init__(self,
            nhid,
            edge_emd_dim,
            mlp_layers=1,
        ...
        if elene_embedder is not None:
            self.elene_embedder = elene_embedder
        else:
            self.elene_embedder = ELENEEmbedder(max_degree, max_distance, use_elene_degrees, joint_embeddings)
        self.subgraph_transform = MLP(2 * nhid + self.elene_embedder.node_embedder_size, nhid, nlayer=mlp_layers, with_final_activation=True)
        if not self.elene_embedder.node_only:
            self.relative_transform = MLP(2 * nhid + edge_emd_dim + self.elene_embedder.edge_embedder_size, edge_emd_dim, nlayer=mlp_layers, with_final_activation=True)
    ...
    def reset_parameters(self):
        self.elene_embedder.reset_parameters()
    ...
    def forward(self, data):
        ...
        # Compute ELENE features for the subgraph nodes and edges
        node_distance_mask = (data.subgraph_node_hops <= self.max_distance).reshape(-1, 1)
        if self.elene_embedder.node_only:
            node_elene_emb = self.elene_embedder(data)
            edge_elene_emb = None
        else:
            ...
            node_elene_emb, edge_elene_emb = self.elene_embedder(data)
        ...
        subgraph_message = torch.cat([root_features, node_features, node_elene_emb], dim=-1)
        ...
        if edge_elene_emb is not None:
            ...
            # The message is the edge messages (root features, edge features, aggregated edges, ELENE degrees and deltas) merged
            edge_message = torch.cat([ego_source_x, combined_subgraphs_edge_attr, edge_messages, edge_elene_emb], dim=-1)
            ...
            # Produce a replacement for node and edge features _with_ ELENE encodings
        ...
        # Produce a replacement for node features _with_ ELENE encodings
