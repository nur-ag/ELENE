...
from core.data import RandomBarabasiAlbertDataset, calculate_stats
...
def create_dataset(cfg): 
    ...
    root = 'data/prefattach'
    # Dirty hack: m must be < 100, n has to be >= 100 and it's the exponent of 10^(n / 100).
    m = cfg.task % 100
    n = round(10 ** ((cfg.task // 100) / 100))
    dataset = RandomBarabasiAlbertDataset(root, n=n, m=m, h=cfg.subgraph.hops, seed=cfg.seed, transform=transform)
...
if __name__ == '__main__':
    # get config 
    cfg.merge_from_file('train/configs/prefattach.yaml')
    cfg = update_cfg(cfg)
