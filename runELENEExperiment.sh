...
ELENE_FIRST="(0,)"
ELENE_HALF=`seq 0 $(( $HALF_LAYERS - 1 )) | tr '\n' ',' | sed 's/\(.*\),$/(\1)/g'`;
...
do
  ...
  do
    ...
    LAYER_LAYOUTS="$ELENE_FIRST $ELENE_HALF $ELENE_FULL"
    ...
    do
      ...
      do
        ...
        do
          ...
          JOB_COMMAND="python -m train.${PROBLEM} model.gnn_type ${GNN_TYPE} elene.model_type ${MODEL_TYPE} elene.max_distance ${MAX_DISTANCE} ${DEGREE_ARGS} elene.layer_indices ${LAYER_LAYOUT} ${GNN_MINI_LAYER_CFG}"
          ...
          # Only run the no-GNN once if we are not in the full ELENE layout
